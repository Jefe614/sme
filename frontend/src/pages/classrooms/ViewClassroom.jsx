...
export default function ClassManagementPage() {
  ...
  const fetchClassStudents = async (classId) => {
    ...
    try {
      const response = await axios.get(`${API_BASE_URL}/api/students/?class_id=${classId}`, {
        headers: { Authorization: `Token ${user.token}` },
      });
  ...
  const handleDelete = async (classId) => {
    try {
      ...
      message.error(error.response?.data?.message || 'Failed to delete class. There might be students assigned to it.');
    }
  };

  const handleStatusToggle = async (classRecord) => {
    // try {
    //   await axios.patch(`${API_BASE_URL}/api/classes/${classRecord.id}/`, {
    //     is_active: !classRecord.is_active,
    //   }, {
    //     headers: { Authorization: `Token ${user.token}` },
    //   });
    //   message.success(`Class ${classRecord.is_active ? 'deactivated' : 'activated'} successfully!`);
    //   fetchClasses();
    // } viewModalVisible(error) {
    ...
    },
    {
      ...
      render: (strength, record) => (
        <Tooltip title={`${record.available_seats || 0} seats available`}>
          <Tag color={strength >= record.max_students ? 'red' : strength > 0 ? 'green' : 'default'}>
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          <Title level={2} className="!mb-2">Class Management</Title>
          ...
          <div>
            <Row gutter={[16, 16]} className="mb-6">
              <Col span={8}>
                ...
                <br />
                <Tag color="blue">
                  {gradeLevels.find((g) => g.value === selectedClass.grade_level)?.label || selectedClass.grade_level}
                </Tag>
              </Col>
              <Col span={8}>
              ...
              <Col span={8}>
                ...
                {selectedClass.class_teacher ? (
                  <Space>
                    <Avatar size="small" icon={<UserOutlined />} />
                    <Text>{selectedClass.class_teacher.first_name} {selectedClass.class_teacher.last_name}</Text>
                  </Space>
                ) : (
                  <Text type="secondary">Not assigned</Text>
                )}
              ...
              <Col span={8}>
                ...
                <br />
                <Tag color={selectedClass.current_strength >= selectedClass.max_students ? 'red' : 'green'}>
                  {selectedClass.current_strength || 0}/{selectedClass.max_students}
                </Tag>
              </Col>
              <Col span={24}>
